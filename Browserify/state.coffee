...
state.lastUpdate = 0
state.BaseRotationEuler = new THREE.Euler()#new THREE.Vector3()
...
state.open = (app, controls) ->
    ...
    if app.status is 'ready'
        console.log('starting..')
        state.start(app, controls)
    else
...
#        debugger;
        ...
        $.post('http://vos.jit.su/newAppUsed?token=' + token, {appID: app._id})
...
state.start = (app, controls) ->
    ...
    for loaded in state.apps
        if loaded._id is app._id and loaded.status is 'started'
            other = loaded
...
state.drawPanel = (scene, util) ->
    ...
    if state.socket? and every % 5 is 0 #Limiting in case this is too much to handle
